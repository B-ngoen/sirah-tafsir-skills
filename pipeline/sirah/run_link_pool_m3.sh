...
one(){ b=$1; for try in 1 2 3 4; do
  ...
  pi --provider minimax --model minimax-m3 -p --no-session "Baca data/LINK-LLM-SPEC.md, lalu kerjakan untuk batch $b: baca data/link_batches/$b.txt dan tulis data/link_results/$b.json" > "logs/link/${b}_m3_r$try.out" 2>&1
  [ -s "data/link_results/$b.json" ] && { echo "[$(date '+%T')] ok $b (try $try)"; return 0; }
  sleep $((90*try)); done; echo "[$(date '+%T')] GAGAL $b"; return 1; }
export -f one
for f in data/link_batches/*.txt; do b=$(basename "$f" .txt); [ -s "data/link_results/$b.json" ] || echo "$b"; done | tac | xargs -P 4 -I{} bash -c 'one {}'
