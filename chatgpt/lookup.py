#!/usr/bin/env python3
"""lookup.py — CLI query tafsir verbatim dari tafsir_full.db (versi ChatGPT Custom GPT).

Sama dengan tafsir-lookup/scripts/lookup.py, hanya resolusi DB yang disesuaikan
untuk Code Interpreter ChatGPT (file Knowledge Custom GPT dimount di /mnt/data):
urutan pencarian /mnt/data PALING DULU, cache ekstrak ke /tmp/tafsir-lookup/,
auto-download dicoba paling akhir (sandbox ChatGPT biasanya tanpa internet).

Usage:
    python /mnt/data/lookup.py 2:255                        # semua sumber (markdown)
    python /mnt/data/lookup.py 2:255 -s tabari,maraghi      # filter sumber
    python /mnt/data/lookup.py 2:255 --format json          # output JSON
    python /mnt/data/lookup.py 2:255 --max-chars 3000       # batas potong per sumber (0 = penuh)
    python /mnt/data/lookup.py 1 --intro                    # segmen intro/pembuka surah 1
    python /mnt/data/lookup.py --coverage 2                 # ringkasan cakupan surah 2 per sumber

    python /mnt/data/lookup.py 2:255 --toc                    # daftar segmen + sub-heading
    python /mnt/data/lookup.py 1 --intro --toc                # toc segmen intro surah 1
    python /mnt/data/lookup.py 2:255 --seg 7928 --paras 0-30  # paragraf 0-30 segmen, teks
...
Portabel: DB dicari otomatis (/mnt/data Knowledge Custom GPT -> env TAFSIR_DB
-> assets skill -> cwd -> upload Cowork/claude.ai -> path laptop -> unduh
otomatis dua sumber dicoba TERAKHIR: GitHub release, lalu server privat owner);
bila ketemu .zip/.xz diekstrak sekali ke cache temp.
...
# --- Resolusi DB (versi ChatGPT: /mnt/data Knowledge GPT paling dulu) ---
SKILL_DIR = Path(__file__).resolve().parent.parent
LOCAL_DB = os.environ.get("TAFSIR_LOCAL_DB", "")  # opsional
COWORK_DIRS = ("/mnt/user-data/uploads", "/mnt/user-data", str(Path.home() / "uploads"))
# File Knowledge Custom GPT dimount di sini oleh Code Interpreter ChatGPT.
GPT_MOUNT_DIR = Path("/mnt/data")
VAULT_DIR = Path(os.environ.get("TAFSIR_CACHE_ROOT") or "/nonexistent")


def _cache_dir() -> Path:
    """Folder cache untuk DB hasil ekstrak (versi ChatGPT).

    Urutan: vault (laptop pemilik) -> LOCALAPPDATA (Windows) -> XDG_DATA_HOME
    (Linux, hanya bila env di-set) -> /tmp/tafsir-lookup (sandbox ChatGPT —
    selalu writable, dipakai bila LOCALAPPDATA/XDG tak ada) -> tempdir
    (upaya terakhir). Catatan: isi /tmp tidak persisten antar sesi sandbox,
    jadi ekstraksi xz berulang tiap sesi baru (±1 menit).
    """
    cands = []
    if VAULT_DIR.is_dir():
    ...
    if os.environ.get("XDG_DATA_HOME"):
        cands.append(Path(os.environ["XDG_DATA_HOME"]) / "tafsir-lookup")
    fallback = Path("/tmp") / "tafsir-lookup"  # sandbox ChatGPT (Linux)
    cands.append(fallback)
    tmp = Path(tempfile.gettempdir()) / "tafsir-lookup"
    if tmp != fallback:
        cands.append(tmp)
    for c in cands:
        try:
...
def fail_db_missing(path=None, autodl_failed=False):
    ...
    if autodl_failed:
        msg += (
            "\n(auto-download dari kedua sumber juga gagal — sandbox ChatGPT biasanya"
            " tanpa internet; solusi: unggah tafsir_full.db.xz ke Knowledge)"
        )
    msg += (
        "\n- Di Custom GPT (ChatGPT): unggah tafsir_full.db.xz (18.7 MB, format utama;"
        " .zip 34 MB juga diterima) ke Knowledge GPT ini (Configure → Knowledge),"
        " lalu mulai percakapan BARU dan ulangi permintaan."
        "\n- Alternatif: set env TAFSIR_DB ke path .db/.xz/.zip yang valid, atau"
        " jalankan dengan --db PATH."
    )
    fail(msg, code=3)
...
def find_db():
    ...
    Urutan (versi ChatGPT): (a) /mnt/data — file Knowledge Custom GPT,
    PALING DULU (.db/.xz/.zip); (b) env TAFSIR_DB — autoritatif, bila di-set
    tapi file tidak ada maka open_db() exit 3; (c) assets di folder skill;
    (d) cwd; (e) folder upload Cowork/claude.ai (glob tafsir_full.db*);
    (f) path laptop pemilik; (g) auto-download — GitHub release lalu server
    privat owner, dicoba PALING AKHIR karena sandbox ChatGPT biasanya tanpa
    internet, sebelum fail_db_missing().
    Env TAFSIR_LOOKUP_SKIP_LOCAL=1 melewati jalur a-f dan langsung ke (g) —
    untuk testing/simulasi tanpa file lokal.
    """
    # (a) /mnt/data — Knowledge Custom GPT, PALING DULU
    for name in ("tafsir_full.db", "tafsir_full.db.xz", "tafsir_full.db.zip"):
        c = GPT_MOUNT_DIR / name
        if c.is_file():
            return c
    # (b) env TAFSIR_DB — override eksplisit pengguna
...
def download_db():
    ...
    Versi ChatGPT: ini jalur PALING TERAKHIR find_db() — sandbox ChatGPT
    biasanya tanpa internet, jadi bila kedua sumber gagal jawabannya adalah
    meminta user mengunggah tafsir_full.db.xz ke Knowledge (fail_db_missing).
    Sumber dicoba berurutan — sumber 1 GitHub release, sumber 2 server privat
    owner; sumber pertama yang sukses dipakai. Return Path DB cache, atau
    None bila semua gagal (server mati / tanpa internet). Chunked 1 MB,
    progress tiap 25% ke stderr, timeout koneksi 15 detik. URL tidak pernah
    dicetak ke stderr/log — pesan hanya "sumber unduhan 1/2".
    """
...
def main():
    ...
    ap.add_argument("--db", help="path DB SQLite, .zip, atau .xz (default: dicari otomatis — /mnt/data Knowledge GPT -> env TAFSIR_DB -> assets skill -> cwd -> upload Cowork -> path laptop -> auto-download)")
    args = ap.parse_args()
