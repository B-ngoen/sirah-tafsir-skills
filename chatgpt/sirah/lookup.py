#!/usr/bin/env python3
"""lookup.py — CLI query verbatim sirah_full.db (versi ChatGPT Custom GPT / Projects).

Sama dengan skills/sirah-lookup/scripts/lookup.py; hanya resolusi DB yang disesuaikan
untuk Code Interpreter ChatGPT (file Knowledge/Project dimount di /mnt/data):
/mnt/data dicek PALING DULU, cache ekstrak ke /tmp/sirah-lookup/, auto-download
dicoba paling akhir (sandbox ChatGPT biasanya tanpa internet).
...
GPT_MOUNT_DIR = Path("/mnt/data")  # file Knowledge Custom GPT / Project dimount di sini
...
def _cache_dir() -> Path:
    ...
    if os.environ.get("XDG_DATA_HOME"):
        cands.append(Path(os.environ["XDG_DATA_HOME"]) / "sirah-lookup")
    fallback = Path("/tmp") / "sirah-lookup"  # sandbox ChatGPT (Linux)
    cands.append(fallback)
    tmp = Path(tempfile.gettempdir()) / "sirah-lookup"
    if tmp != fallback:
        cands.append(tmp)
    for c in cands:
        try:
...
def fail_db_missing(path=None, autodl_failed=False):
    ...
    msg += (
        "\n- Di ChatGPT: unggah sirah_full.db.xz (17 MB) ke Knowledge Custom GPT atau file Project,"
        " lalu mulai percakapan baru."
...
def find_db():
    ...
    """
    for n in ("sirah_full.db", "sirah_full.db.xz", "sirah_full.db.zip"):
        p = GPT_MOUNT_DIR / n
        if p.is_file():
            return p
